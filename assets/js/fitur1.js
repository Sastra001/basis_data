...
function showAssetChart() {
  ...
  drawSidebarCharts(currentAsset);
...
function renderBadges() {
  ...
  // Tambahkan toggle sidebar button di mobile
  if (window.innerWidth < 640 && (Object.keys(activeIndicators).length > 0 || document.getElementById('f1-result').offsetParent !== null)) {
    badges.unshift(`
      <div class="badge" style="background:#e8f0fe;border-color:#b3cef5;cursor:pointer;" onclick="toggleMobileSidebar()">
        <span style="color:#185FA5;font-weight:600;">📋 Daftar</span>
      </div>
    `);
  }
